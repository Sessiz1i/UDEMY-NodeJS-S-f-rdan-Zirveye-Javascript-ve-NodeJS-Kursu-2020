// View js
const  getImg   = () => import("./unsplash_api");
const  getJoke  = () => import("./joke_api");
const  getTrans = () => import("./translate_api");
...
class View {
    ...
    async joke() {
        const image = await (await getImg()).img()
        const joke  = await (await getJoke()).joke()
        const trans = await (await getTrans(joke)).translater()
