...
class View {
    ...
    cancel(){
        ...
        if (this.name.value.length >0 || this.surname.value.length >0 || this.email.value.length >0){
    ...
    formSubmit(e) {
        ...
        const newData = new Data(this.name.value, this.surname.value, this.email.value, this.formBtn.value);
        (newData && result.status) ? result = Validation.freeSpace(newData) : null;
        (newData && result.status) ? result = Validation.stringFixed(newData) : null;
        (newData && result.status) ? result = Validation.validateEmail(newData.email) : null;
        (newData && result.status) ? result = this.store.findData(newData.email): null;
        // Validation Başarılı
        if (result.status) {
            if (newData.formBtn === "create") {
                ...
                this.clearForm();
                this.statusMessage({status: true, message: `İsim: ${newData.name.fontcolor('blue')} Soyisim: ${newData.surname.fontcolor('blue')}<br>E-mail: ${newData.email.fontcolor('blue')} olarak yeni bir kişi eklendi.`});
                ...
                // Kayıttan sonra form iputlarını boşaltır
                this.clearForm();
                this.statusMessage({status: true, message: `İsim: ${newData.name.fontcolor('blue')} Soyisim: ${newData.surname.fontcolor('blue')}<br>E-mail: ${newData.email.fontcolor('blue')} olarak güncellendi.`});
            }
        } else {
    ...
    crudData(e) {
        if (e.target.id === "edit") {
            ...
            this.dataDeleteView();
            this.clearForm();
            this.statusMessage({status: true, message: `İsim: ${this.dataTr.cells[0].textContent.fontcolor('blue')} Soyisim: ${this.dataTr.cells[1].textContent.fontcolor('blue')}<br>E-mail: ${this.dataTr.cells[2].textContent.fontcolor('blue')} bilgilerine sahip kişi silindi.`});
        }
    }
    ...
    statusMessage(result) {
        ...
        info.classList.add(result.status ? "alert-success" : "alert-danger", result.status ? "text-success" : "text-danger");
        info.setAttribute("role", "alert")

        info.innerHTML = `<i class="fa ${result.status ? "fa-check " : "fa-times"}">&nbsp;</i>${result.message}`;
...
class Validation {
    ...
    static freeSpace(datas) {
        ...
        for (var data in datas) {
            if (datas[data].length === 0 || datas[data].length[1] === " ") {
                ...
                return {status: false, message: `${data} alanı veya  boş bırakılamaz.`}
            }
        }
    ...
    static validateEmail(email) {
        const regEx = /^(([^<>()[\]\\.,;:ıİğĞüÜşŞöÖçÇ\s@"]+(\.[^<>()[\]\\.,;:ıİğĞüÜşŞöÖçÇ\s@"]+)*)|(".+"))@((\[[0-9ıİğĞüÜşŞöÖçÇ]{1,3}\.[0-9ıİğĞüÜşŞöÖçÇ]{1,3}\.[0-9ıİğĞüÜşŞöÖçÇ]{1,3}\.[0-9ıİğĞüÜşŞöÖçÇ]{1,3}\])|(([a-zA-Z\-0-9]+\.)+[a-zA-Z]{2,}))$/;
        if (regEx.test(String(email))) {
            return {status: true, message: "Sikinti yok."}
        } else {
            return {status: false, message: "Geçersiz bir e-posta adresi girdiniz!"}
        }
    }
...
class Store {
    ...
    findData(email) {
        if (this.allData.find(data => data.email === email)) {
            return {status:false, message:`${email.fontcolor("blue")} Önceden kayıt edilmiş bir E-mail dir.`}
        }else{
            return {status:true}
        }
    }
