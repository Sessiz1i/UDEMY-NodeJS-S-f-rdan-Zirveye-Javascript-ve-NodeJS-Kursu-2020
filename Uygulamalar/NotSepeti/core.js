...
function gorevVaidate(e) {
    if (yeniGrvIpt.value.length >= 0 && yeniGrvIpt.value.length < 5 || yeniGrvIpt.value.length > 25) {
...
function gorevSilTmm(e) {
    ...
    if (clickElement.classList.contains('btn-tmm')) {
        ...
        if (confirm('Görevi Silmek istediğinize eminmisiniz?')) {
            ...
            clickElement.parentElement.addEventListener('transitionend', _ => clickElement.parentElement.remove())
        } else {
